...
package com.falstaff.core.alignable.signature.pattern;
...
{
	private static final Logger log = LoggerFactory.getLogger(Pattern.class.getName());
    ...
    private static String FILE_START = "_#FS#_";
    private static String FILE_END = "_#FE#_";
    
    public static class KtupTreeInfo {
    	int headMaxLength = 0;
    	int tailMaxLength = 0;
		...
		public void setHeadMaxLength(int headMaxLength) {
			this.headMaxLength = headMaxLength;
		}

		public int getTailMaxLength() {
			return tailMaxLength;
		}

		public void setTailMaxLength(int tailMaxLength) {
			this.tailMaxLength = tailMaxLength;
		}
		...
		public void setMaxBuffer(int[] maxBuffer) {
			if (this.maxBuffer != null) {
				headMaxLength += this.maxBuffer.length + tailMaxLength;
				tailMaxLength = 0;
			}
		...
		public void addLength(int length) {
			if (maxBuffer == null) {
				headMaxLength += length;
			}
			else {
				tailMaxLength += length;
			}
		}
    }
    
    public KtupTreeInfo getKtupTreeInfo() {
    	KtupTreeInfo ktupTreeInfo = new KtupTreeInfo();
    	matchRoot.getKtupTreeInfo(ktupTreeInfo);
    	return ktupTreeInfo;
    }
    
    public TreeInfo getTreeInfo() {
    	TreeInfo treeInfo = new TreeInfo();
    	matchRoot.study(treeInfo);
    	return treeInfo;
    }
    ...
    public Automaton getAutomaton() {
    	FSA fsa = new FSA();
    	matchRoot.getFSA(fsa);
    	Automaton automaton = fsa.getAutomaton();
    	/*
    	if (automaton != null) {
    		automaton.minimize();
    	}
...
loop:   for(int x=0, offset=0; x<nCodePoints; x++, offset+=len) {
    ...
    static class Node extends Object {
        ...
        void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
            if (next != null) {
                next.getKtupTreeInfo(ktupTreeInfo);
            }     	
        }
    }
    ...
    static class Start extends Node {
		...
		void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
			next.getKtupTreeInfo(ktupTreeInfo);
			ktupTreeInfo.setMaxValid(false);
		}
    }
    ...
    static final class Begin extends Node {
		...
		@Override
		public void getFSA(FSA fsa) {
			Automaton automaton = Automaton.makeString(FILE_START);
			if (next != null) {
				FSA nextFSA = new FSA();
				next.getFSA(nextFSA);
				Automaton nextAutomaton = nextFSA.getAutomaton();
				if (nextAutomaton != null) {
					automaton = automaton.concatenate(nextAutomaton);
				}
			}
			fsa.setAutomaton(automaton);
		}       
    ...
    static final class Dollar extends Node {
		...
		@Override
		public void getFSA(FSA fsa) {
			Automaton automaton = Automaton.makeString(FILE_END);
			if (next != null) {
				FSA nextFSA = new FSA();
				next.getFSA(nextFSA);
				Automaton nextAutomaton = nextFSA.getAutomaton();
    ...
    static final class Single extends BmpCharProperty {
		...
		@Override
		public void getFSA(FSA fsa) {
			Automaton automaton = Automaton.makeChar((char) c);
			if (next != null) {
				FSA nextFSA = new FSA();
				next.getFSA(nextFSA);
				Automaton nextAutomaton = nextFSA.getAutomaton();
    ...
    static class SliceNode extends Node {
        ...
        void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
        	if (ktupTreeInfo.getMaxBuffer() == null) {
        		ktupTreeInfo.setMaxBuffer(buffer);
        	}
        	else {
        		if (buffer != null) {
        			if (buffer.length > ktupTreeInfo.getMaxBuffer().length) {
        				ktupTreeInfo.setMaxBuffer(buffer);
        			}
        			else {
        				ktupTreeInfo.addLength(buffer.length);
        			}
        		}
        	}
        	next.getKtupTreeInfo(ktupTreeInfo);
		...
		public void getFSA(FSA fsa) {
			...
			for (int i = 0; i < buffer.length ; i++) {
				if (i == 0) {
					automaton = Automaton.makeChar((char)buffer[i]);
				}
				else {
					automaton = automaton.concatenate(Automaton.makeChar((char)buffer[i]));
				}
			}
			if (next != null) {
				FSA nextFSA = new FSA();
    ...
    private static CharProperty rangeFor(final int lower,
        ...
        return new CharProperty() {
				...
				@Override
				public void getFSA(FSA fsa) {
					Automaton automaton = Automaton.makeCharRange((char)lower, (char)upper);
					if (next != null) {
						FSA nextFSA = new FSA();
						next.getFSA(nextFSA);
						Automaton nextAutomaton = nextFSA.getAutomaton();
    ...
    static final class All extends CharProperty {
		...
		@Override
		public void getFSA(FSA fsa) {
			Automaton automaton = Automaton.makeAnyChar();
			if (next != null) {
				FSA nextFSA = new FSA();
				next.getFSA(nextFSA);
				Automaton nextAutomaton = nextFSA.getAutomaton();
    ...
    static final class Ques extends Node {
		...
		@Override
		void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
			atom.getKtupTreeInfo(ktupTreeInfo);
			next.getKtupTreeInfo(ktupTreeInfo);
		}
    }
    ...
    static final class Curly extends Node {
		...
		void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
            ...
            if (maxV & info.maxValid) {
                int temp = info.maxLength * cmax + maxL;
                info.maxLength = temp;
                if (temp < maxL) {
                    info.maxValid = false;
                }
	        ...
	        ktupTreeInfo.addLength(info.maxLength);
	        ktupTreeInfo.setMaxValid(info.maxValid);
			next.getKtupTreeInfo(ktupTreeInfo);
		}
		...
		public void getFSA(FSA fsa) {
			...
			FSA atomFSA = new FSA();
			atom.getFSA(atomFSA);
			Automaton atomAutomaton = atomFSA.getAutomaton();

			if (atomAutomaton != null) {
				if (cmin == 0 && cmax == MAX_REPS) {
					atomAutomaton = atomAutomaton.repeat();
				}
				if (cmin > 0 && cmax == MAX_REPS) {
					atomAutomaton = atomAutomaton.repeat(cmin);
				}
				if (cmin >= 0 && cmax < MAX_REPS) {
					atomAutomaton = atomAutomaton.repeat(cmin, cmax);
				}
				...
				if (next != null) {
					...
					Automaton nextAutomaton = nextFSA.getAutomaton();
					if (nextAutomaton != null) {
						atomAutomaton = atomAutomaton.concatenate(nextAutomaton);
					}
				}
				fsa.setAutomaton(atomAutomaton);
			}
    ...
    static final class Branch extends Node {
		...
		void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
            ...
            ktupTreeInfo.addLength(maxL);
            ktupTreeInfo.setMaxValid(ktupTreeInfo.isMaxValid() && maxV);
            
            conn.next.getKtupTreeInfo(ktupTreeInfo);
		}

		@Override
		public void getFSA(FSA fsa) {
			List<Automaton> nextAutomatons = new LinkedList<Automaton>();
		
			for (int n = 0; n < size; n++) {
				FSA nextFSA = new FSA();
				Automaton nextAutomaton;
				if (atoms[n] != null) {
					atoms[n].getFSA(nextFSA);
				}
				else {
					nextFSA.setAutomaton(Automaton.makeEmptyString());
				}
				nextAutomaton = nextFSA.getAutomaton();
				if (nextAutomaton != null) {
					nextAutomatons.add(nextAutomaton);
				}
			}
			
			Automaton automaton = null;
			if (nextAutomatons.size() > 0) {
				automaton = Automaton.union(nextAutomatons);
			}
			
			FSA nextFSA = new FSA();
			conn.next.getFSA(nextFSA);
			Automaton nextAutomaton = nextFSA.getAutomaton();
    ...
    static final class Prolog extends Node {
		...
		@Override
		void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
			loop.getKtupTreeInfo(ktupTreeInfo);
		}
    }
    ...
    static final class Conditional extends Node {
		...
		void getKtupTreeInfo(KtupTreeInfo ktupTreeInfo) {
	        ...
	        ktupTreeInfo.addLength(maxL + Math.max(maxL2, info.maxLength));
	        ktupTreeInfo.setMaxValid(maxV & maxV2 & info.maxValid);
			next.getKtupTreeInfo(ktupTreeInfo);
		}
